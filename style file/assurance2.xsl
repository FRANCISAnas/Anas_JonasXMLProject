...
<!-- Les clients qui ont au moins 2 contrat d'assurance dans la base de données -->
    <xsl:template match="CampagnieAssurance">
        ...
        <xsl:variable name="contractsNumber" select="2"/>
        <head>
    ...
    <xsl:template match="Client">
        ...
        <div class="w3-col s4">
            <div class="w3-container w3-teal">
                ...
                <h1><xsl:value-of select="translate(Nom, $smallcase, $uppercase)"></xsl:value-of> <xsl:text> </xsl:text> <xsl:value-of select="Prenom"></xsl:value-of></h1>
            </div>
            <center>
